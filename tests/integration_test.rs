...
use rusted_tools::http::handlers::AppState;
...
async fn create_test_app() -> Router {
    ...
    let manager = Arc::new(rusted_tools::server::ServerManager::new());
    
    // Initialize servers from config
    manager.init_from_config(config.mcp_servers.clone()).await.unwrap();
    
    let router = Arc::new(rusted_tools::proxy::Router::new(manager.clone()));
    router.init_from_config(&config.mcp_servers).unwrap();
    
    let state = AppState { manager, router };
    
    Router::new()
        .merge(rusted_tools::http::routes::health_routes())
        .merge(rusted_tools::http::routes::management_routes())
        .merge(rusted_tools::http::routes::mcp_routes())
        .with_state(state)
}
